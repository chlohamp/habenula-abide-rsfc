...
def _get_parser():
    ...
    parser.add_argument(
        ...
        dest="rois_dir",
        required=False,
        help="Directory containing subject-level ROI nifti files (optional)",
    )
    parser.add_argument(
        "--n_jobs",
        dest="n_jobs",
...
def main(mriqc_dir, clean_dir, rsfc_dir, subject, sessions, space, desc_list, rois, rois_dir=None, n_jobs=4):
    ...
    for session in sessions:
        ...
        for clean_subj_file in clean_subj_files:
            ...
            print(f"\tSubject: {prefix}", flush=True)
            ...
            # Determine which ROIs to use for this subject: either the
            # provided global list (`rois`) or subject-specific files in
            # `rois_dir` (if given).
            if rois_dir is not None:
                subject_rois = sorted(glob(op.join(rois_dir, f"{subject}*nii*")))
                if len(subject_rois) == 0:
                    print(f"\t\tNo subject-specific ROIs found for {subject} in {rois_dir}", flush=True)
                    # skip this run if no subject ROI is available
                    continue
                rois_for_subject = subject_rois
            else:
                rois_for_subject = rois

            print(f"\tProcessing {subject}, {session}, {rois_for_subject} files:", flush=True)
            ...
            exclude = False
            stim_info = ""
            for i, roi in enumerate(rois_for_subject):
                num = i + 1
