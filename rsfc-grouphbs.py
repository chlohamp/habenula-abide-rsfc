...
def main(
    ...
    # If a habenula-rois subject list exists, limit the analysis to those subjects
    rois_subjects_tsv = op.join(dset, "seed-regions", "habenula-rois", "habenula-rois_subjects.tsv")
    valid_subjects = None
    if op.exists(rois_subjects_tsv):
        try:
            rois_df = pd.read_csv(rois_subjects_tsv, sep="\t", dtype=str, comment="#", keep_default_na=False)
            # prefer a column named 'subject' or 'participant_id' if present
            if "subject" in rois_df.columns:
                col = "subject"
            elif "participant_id" in rois_df.columns:
                col = "participant_id"
            else:
                col = rois_df.columns[0]
            # extract and normalize to 'sub-XXXX' format
            subjects_raw = rois_df[col].astype(str).tolist()
            valid_subjects = []
            for s in subjects_raw:
                if s.strip() == "":
                    continue
                ss = s.strip()
                # If filename was provided, try to extract sub- pattern
                if "sub-" in ss:
                    import re

                    m = re.search(r"(sub-[A-Za-z0-9_-]+)", ss)
                    if m:
                        ss = m.group(1)
                # Ensure prefix
                if not ss.startswith("sub-"):
                    ss = f"sub-{ss}"
                valid_subjects.append(ss)
            valid_subjects = list(dict.fromkeys(valid_subjects))
            print(f"Limiting group to {len(valid_subjects)} subjects from {rois_subjects_tsv}", flush=True)
        except Exception:
            valid_subjects = None
    ...
    # If a subject list was provided via habenula-rois, filter the file lists
    if valid_subjects is not None:
        prefixes_tpl = tuple(valid_subjects)
        clean_briks_files = [x for x in clean_briks_files if op.basename(x).startswith(prefixes_tpl)]
        clean_mask_files = [x for x in clean_mask_files if op.basename(x).startswith(prefixes_tpl)]
    ...
    # Reconcile subjects present in both briks and masks. This avoids a hard
    # assertion failure when some subjects are missing masks or briks.
    subj_briks = set([op.basename(x).split("_")[0] for x in clean_briks_files])
    subj_masks = set([op.basename(x).split("_")[0] for x in clean_mask_files])
    common_subj = subj_briks.intersection(subj_masks)
    if valid_subjects is not None:
        # further restrict to the requested subjects from habenula-rois TSV
        common_subj = common_subj.intersection(set([s.replace('sub-','sub-') for s in valid_subjects]))

    if len(common_subj) == 0:
        raise RuntimeError(f"No overlapping subjects found between briks and masks after filtering. briks:{len(subj_briks)} masks:{len(subj_masks)}")

    # Filter and sort file lists to include only common_subjects
    common_prefixes = tuple(sorted(common_subj))
    clean_briks_files = sorted([x for x in clean_briks_files if op.basename(x).startswith(common_prefixes)])
    clean_mask_files = sorted([x for x in clean_mask_files if op.basename(x).startswith(common_prefixes)])

    if len(clean_briks_files) != len(clean_mask_files):
        # Provide a helpful diagnostic rather than an opaque assertion
        missing_in_briks = sorted(list(subj_masks - subj_briks))
        missing_in_masks = sorted(list(subj_briks - subj_masks))
        raise RuntimeError(
            "Mismatched counts after reconciliation: \n"
            f"briks: {len(clean_briks_files)}, masks: {len(clean_mask_files)}\n"
            f"subjects_only_in_briks: {missing_in_masks}\n"
            f"subjects_only_in_masks: {missing_in_briks}\n"
        )
    ...
    if not op.exists(f"{onetwottest_briks_fn}+tlrc.BRIK"):
        # Print diagnostic summary: counts per DX group and age statistics
        included_subjects = [op.basename(x).split("_")[0] for x in clean_briks_files]
        subs_df = participants_df[participants_df["participant_id"].isin(included_subjects)].copy()
        if subs_df.empty:
            print("Warning: no participant metadata found for the included subjects", flush=True)
        else:
